...
function OBJ_MOD_INSTALACIONES() {
				...
				if(data=="mysql_no"){
					...
					if(data!='no data'){
						...
						for(var index in respuesta){
							...
							if(!GL_INSTALACIONES[respuesta[index].id_biblioteca]){
								GL_INSTALACIONES[respuesta[index].id_biblioteca]=respuesta[index];
								GL_CONT_INSTALACIONES++;	
   								...
   								if(objeto.admin){
									...
									html_elemento='<elemento data-src="IMG/BIBLIOTECA/'+respuesta[index].id_img+'.'+respuesta[index].ext_img+'" data-msrc="IMG/BIBLIOTECA/'+respuesta[index].id_img+'.'+respuesta[index].ext_img+'" data-espacio="1" data-titulo="" data-subtitulo="'+respuesta[index].nombre_biblio+' ('+respuesta[index].num_archivos+')">'+

           								 '<ClassExtra DOMdestino="cg-elemento" class="trans_bezier_05 '+trans_delay+' anim-escala_0"></ClassExtra>'+
           								 '<AttrExtra DOMdestino="cg-elemento" atributos="data-id_biblioteca='+"'"+respuesta[index].id_biblioteca+"'"+ '"></AttrExtra>'+
   						...
   						if(!objeto.admin){
							var delay_ini=setInterval(function(){
								...
								if(GL_COMPONENTE_GALERIA){
									...
									GL_COMPONENTE_GALERIA.ini_galeria_por_id($('#portafolios .comp-galeria'));
    ...
    this.get_html_instalacion=function(instalacion){
    	...
    	if(!fun_esblanco(instalacion.id_img)){
	  	...
	  	html_instalacion= '<article id="instalacion-'+instalacion.id_biblioteca+'" data-id="'+instalacion.id_biblioteca+'" class="instalacion transition_04" data-item="-1">'+
			...
			if(this.admin){
				html_instalacion+='<input type="button" class="btn_eliminar" data-id_instalacion="'+instalacion.id_biblioteca+'" value="Eliminar"/><input type="button" class="btn_modificar" data-id_instalacion="'+instalacion.id_biblioteca+'" value="Modificar"/>';
			}
